...
`Index only scan` здесь не помог, потому что `rows=5000000` как было, так и осталось. **Без изменения запроса решить задачу не удалось.** А так, конечно, верный запрос был бы следующий(с индексом на `t2.day`):
```sql
